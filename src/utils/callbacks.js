...
const setCallbacks = reloadChannelsFN => {
  Lumino.callbacks.set.setOnReceivedPaymentCallback(
    CALLBACKS.RECEIVED_PAYMENT,
    () => {
      showInfo("Received a payment, now processing it...");
    }
  );

  Lumino.callbacks.set.setOnCompletedPaymentCallback(
  ...
  Lumino.callbacks.set.setOnOpenChannelCallback(
    CALLBACKS.OPEN_CHANNEL,
    channel => {
      const {
      ...
      for (let notifierEndpoint of notifierEndpoints) {
        Lumino.get().actions.subscribeToUserClosesChannelOnToken(
          notifierEndpoint,
          token_network_identifier
        );
  ...
  Lumino.callbacks.set.setOnChannelDepositCallback(
    CALLBACKS.DEPOSIT_CHANNEL,
    channel => {
      const {
        channel_identifier: chId,
        partner_address: p,
  ...
  Lumino.callbacks.set.setOnRequestClientOnboarding(
  ...
  Lumino.callbacks.set.setOnClientOnboardingSuccess(
